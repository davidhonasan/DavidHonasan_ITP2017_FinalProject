...
from django.contrib.auth.decorators import \
    login_required  # importing modules for redirecting to login page if entering page that needs login
...
def transfer(request):
    ...
    try:
        ...
        if u_receive != request.user.username:
            ...
            if amt <= acc_sender.balance and amt >= 10000:
                ...
                messages.error(request,
                               'You don\'t have that much amount of balance or trying to transfer below the minimum amount. You only have Rp. {}'.format(
                                   acc_sender.balance))
                return redirect('bank:transfer_form')
        else:
        ...
        messages.error(request,
                       'You can\'t transfer to \'{}\' because that user doesn\'t exist.'.format(request.POST['to']))
        return redirect('bank:transfer_form')
